...
class camera {
  public:
    ...
    void render(const hittable& world) {
        ...
        parallel_for(0, image_height, 1, [&](int j) {
#else
        for (int j = 0; j < image_height; ++j) {
#endif

#if ENABLE_PARALLEL_FOR && ENABLE_NESTING_PARALLEL_FOR
            parallel_for(0, image_width, 1, [&](int i) {
#else
            for (int i = 0; i < image_width; ++i) {
                ...
                unsigned char* pixel = &image[(j * image_width + i) * 3];
